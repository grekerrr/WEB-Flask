...
class UserView(MethodView):
    def get(self, user_id):
        ...
        if user is not None:
            ...
            responce = jsonify({'error': 'not found'})
    ...
    def post(self):
        ...
        if not UserModel.query.filter_by(name=user_name).first():
            ...
            responce = jsonify({'error': 'This user already exists'})
            responce.status_code = 400
            return responce
...
class AdvertisementView(MethodView):
    ...
    def post(self):
        ...
        if UserModel.query.filter_by(id=ad_owner).first():
            ...
            responce = jsonify({'error': 'This user does not exist'})
            responce.status_code = 404
            return responce
    def put(self, ad_id):
        ad = AdvertisementModel.query.get(ad_id)
        if ad is not None:
            ...
            if ad_owner:
                if UserModel.query.filter_by(id=ad_owner).first():
                    ...
                    responce = jsonify({'error': 'This owner does not exist'})
                    responce.status_code = 404
                    return responce
            ...
            responce = jsonify({'error': 'This advertisement does not exist'})
            responce.status_code = 404
            return responce
